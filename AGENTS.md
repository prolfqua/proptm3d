...
| `assets/` | The static browser app. `index.html` + `app.js` (composition root: loads the catalog, owns the `Session`, builds tables, 3D panels and the N-to-C figure into the shell's hosts, applies intents). `lib/` holds the DOM-free modules `session.js` (selection state and every derivation: contrast scope, category membership, decoration, rows per contrast), `payload.js` (fetch/decode, cbor-x) and `color.js`; `shell/ptm-app.js` is the Lit shell (tabs Find / Protein, the selects, layouts with hosts; emits `intent` events, decides nothing); `render/` holds the backends `tables.js` (Tabulator columns and construction), `viewer3d.js` (3Dmol styles, site marks, the per-contrast `ViewerPool`) and `plotly.js`; `vendor/` holds pinned CDN re-export shims (lit 3.2.1, cbor-x 1.6.6, tabulator 6.5.2, plotly 4.0.0, same stack as BioBeamer/rawDIAGQC); 3Dmol 2.5.5 is a classic `<script>` from jsdelivr |
